...
var _asq = (0, _asynquence2.default)();
...
Graph.prototype.addBot = function (bot) {
  ...
  Object.keys(bot.inputs).forEach(function (k) {
    ...
    if (typeof value === 'string' || value instanceof String) {
      var patternMatch = value.match(/([A-Z][A-Za-z0-9]+)>([A-Z0-9]+)/);
...
Graph.prototype.addBots = function (_bots) {
  ...
  _bots.map(function (b) {
    return _this.addBot(b);
  });
};
...
Graph.prototype.run = function (callback) {
  ...
  this.sorted.forEach(function (ids) {
    ...
    _asq.all.apply(_asq, _toConsumableArray(ids.map(wrap))); //.then(() => callback())
  });
};
