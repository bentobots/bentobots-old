...
function Graph() {
  ...
  this.name = name;
  this.bots = new _immutable.List();
  this.connections = new _immutable.List();
...
Graph.prototype.run = function (callback) {
  ...
  this.sorted.forEach(function (ids) {
    ...
    (_ASQ = (0, _asynquence2.default)()).all.apply(_ASQ, _toConsumableArray(ids.map(wrap))); //.then(() => callback())
  });
};
