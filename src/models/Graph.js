...
export default function Graph({name = 'Untitled Graph'} = {}) {
  this.name = name
  this.bots = List()
...
Graph.prototype.addBot = function(bot, callback = null) {
  if (!bot) throw new Error('Bot not provided')
  if (bot.graph) throw new Error('Bot was already added to graph')
  if (this.bots.find(b => b.id === bot.id)) {
    throw new Error(`Bot with id '${bot.id}' already in graph`)
  }
  bot.graph = this
  this.graph[bot.id] = []
  ...
  //     const patternMatch = value.match(/([A-Z][A-Za-z0-9]+)>([A-Z0-9]+)/)
  //     if (patternMatch) { // if it matches Process1>PORT, then it's an alias
  //       // this.graph[bot.id].push(patternMatch[1])
  //       // console.log("ADD CONNECTION ", patternMatch[1], patternMatch[2])
  //       // console.log(this.graph[bot.id])
  //     }
  //   }
  // })
...
Graph.prototype.addBots = function(_bots) {
  _bots.map(b => this.addBot(b))
}
...
//   conn.tgtBot.inputs[conn.tgtPort] = [conn.srcBot.id, conn.srcPort].join(">")
...
// Graph.prototype.run = function(callback, debug = false) {
//   const wrap = id => done => {
...
//     }
//     _asq.all(...ids.map(wrap))//.then(() => callback())
//   })
// }
